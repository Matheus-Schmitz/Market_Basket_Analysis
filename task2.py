...
# /home/local/spark/latest/bin/spark-submit task2.py 20 50 ta_feng_all_months_merged.csv results2.csv
...
if __name__ == '__main__':
	...
	csvRDD = csvRDD.filter(lambda row: row != csvHeader).map(lambda row: row.split(',')).map(lambda row: (row[0][1:-1]+'-'+row[1][1:-1], str(int(row[5][1:-1])))) #←←←550s / 570s

	# Convert the input data to the case 1 format
	bskts = csvRDD.groupByKey().map(lambda row: list(row[1])).filter(lambda row: len(row) > threshold_filter)
	...
	candidatesRDD = bskts.mapPartitions(lambda partition: apriori(partition)).reduceByKey(lambda x, y: sorted(set(x + y))).sortBy(lambda candidate: candidate[0]).collect() #←←←550s / 570s / 607s / 537s
	...
	#candidatesRDD = bskts.mapPartitions(lambda partition: apriori(partition)).reduceByKey(lambda x, y: sorted(set(x + y))).collect() #←←←561s / 563s
	#candidatesRDD.sort() #←←←635s/630s
