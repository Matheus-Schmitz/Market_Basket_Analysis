...
// spark-submit --class task2 hw2.jar 1 4 small3.csv scala2.csv
...
object task2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val threshold_filter = args(0).toInt
    val supp = args(1).toInt
    val input_file_path = args(2)
    val output_file_path = args(3)
    //val threshold_filter = 1
    //val supp = 4
    //val input_file_path = "small3.csv"
    //val output_file_path = "scala2.csv"
    ...
    val csvRDD2 = csvRDD1.filter(row => row != csvHeader).map(row => row.split(",")).map(row => (row(0).slice(1, row(0).length-1)+"-"+row(1).slice(1, row(1).length-1), row(5).slice(1, row(5).length-1).toLong.toString))

    // Make sure to use a name which shouldn't conflict with basket namings elsewhere
    var bskts: RDD[List[String]] = csvRDD2.groupByKey().map(row => row._2.toList).filter(row => row.size > threshold_filter)
